...
export default function SpecialtiesFilter() {
    ...
    const handleSpecialtiesChange = (selectedOptions: { value: string; label: string }[]) => {
        setSpecialties(selectedOptions.map(option => option.value))
    }

    const specialtiesOptions = specialtiesArr.map(s => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) }))
    const selectedSpecialties = specialties?.map(s => ({ value: s, label: s.charAt(0).toUpperCase() + s.slice(1) })) || []
