...
export default function UpdateSchedule({ userId, setOpen }: { userId: string, setOpen?: Dispatch<SetStateAction<boolean>> }) {
    ...
    const schedules = useMemo(() => groupSchedules(user?.schedules || []), [user]);
    ...
    const onSubmit = async () => {
        ...
        try {
            ...
            const missingSchedules = updatedDays.filter((day) => !updatedSchedules[day]?.length);
            if (missingSchedules.length > 0) {
                toast.error(`The following days are missing schedules: ${missingSchedules.join(", ")}`);
                return;
            }

            const newSchedules = transformSchedulesToRecords(updatedSchedules, userId);
            ...
            if (JSON.stringify(normalizedOldSchedules) === JSON.stringify(normalizedNewSchedules)) {
                toast.error("The schedules are the same as the current ones. No updates needed.");
                return;
            }
            
            const result = await updateSchedule({ schedules: updatedSchedules, userId })
